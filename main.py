...
def get_hh_vacancies(language):
    ...
    while True:
        ...
        response = requests.get(
            url, headers={"User-Agent": "api-test-agent"}, params=params
        )
...
def get_sj_vacancies(language, key):
    ...
    while True:
        params["page"] = page
        response = requests.get(url, headers={"X-Api-App-Id": key},
                                params=params)
        response.raise_for_status()
        response = response.json()
...
def main():
    ...
    for language in programming_languages:
        sj_salaries, sj_total_vacancies = get_sj_vacancies(language,
                                                           secret_ket)
        sj_statistics[language] = calculate_statistics(sj_salaries,
                                                       sj_total_vacancies)

        hh_salaries, hh_total_vacancies = get_hh_vacancies(language)
        hh_statistics[language] = calculate_statistics(hh_salaries,
                                                       hh_total_vacancies)
